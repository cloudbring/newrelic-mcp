...
export const verifyLogAttributes = (
  logs: LogEntry[],
  ...
  for (const log of logs) {
    for (const attr of requiredAttributes) {
      ...
      if (!hasAttribute) {
        throw new Error(
          `Log entry missing required attribute "${attr}": ${JSON.stringify(log)}`
        );
      }
    }
  }
...
export const verifySpan = (
  span: SpanEntry,
  expected: Partial<SpanEntry>
): void => {
  if (expected.name) {
  ...
  if (expected.events) {
    for (const expectedEvent of expected.events) {
      const found = span.events.some(event => 
...
export const verifyPerformanceMetrics = (
  logs: LogEntry[],
  maxDuration?: number
): void => {
...
export const verifyNewRelicAttributes = (logs: LogEntry[]): void => {
  ...
  for (const log of logs) {
    ...
    const hasNRAttributes = nrAttributes.some(attr => 
      log[attr] !== undefined || 
      log.metadata?.[attr] !== undefined ||
...
export const waitForLogs = async (
  ...
  const logs = logGetter();
  throw new Error(
    `Timeout waiting for logs. Expected ${expectedCount}, got ${logs.length}`
  );
};

// Verify log correlation with spans
export const verifyLogSpanCorrelation = (
  logs: LogEntry[],
  spans: SpanEntry[]
): void => {
...
export const logMatchers = {
  ...
  toHaveValidTraceContext: (logs: LogEntry[]) => {
    const allHaveTrace = logs.every(log => 
      log['trace.id'] || log.metadata?.['trace.id']
    );
    return {
      pass: allHaveTrace,
